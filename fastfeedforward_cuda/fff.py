...
class FFFN_function(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, in_weight, in_bias, out_weight, load_balancing_bias, model_dim, depth, number_of_tree, master_node_width, n_nodes_per_tree):
        init_shape = x.shape
        ...
        fff_cuda.forward(x.view(-1, model_dim), in_weight, in_bias, out_weight, load_balancing_bias, output, activated_nodes, activated_nodes_values, model_dim, depth, master_node_width, number_of_tree, n_nodes_per_tree)
